...
export default function ViewsView({ views, archive, sections }) {
  ...
  return (
    <>
      {views.items.map((it) => {
        const title = it.video?.title ?? it.title;
        const url = it.video?.url ?? it.url;
        const dt = it.video?.published ?? it.date;
        return (
        <section className="card" key={it.channel}>
          <div className="views-head">
            <h3 className="val-h">{it.channel}</h3>
            {url && <a href={url} target="_blank" rel="noreferrer" className="views-vid">{title} ↗</a>}
          </div>
